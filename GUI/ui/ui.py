...
def create_window():
    # Dimensiones fijas para la pantalla de 1024x600
    screen_width = 1024
    screen_height = 600
    
    print(f"Usando resolución fija: {screen_width}x{screen_height}", flush=True)
    
    window = pv.PvWindow(
        title="PyVisual IoT - Sistema de Fumigación",
        width=screen_width,
        height=screen_height,
        bg_color=(255, 255, 255, 1),
        icon=None,
        bg_image=None,
        is_frameless=True,  # Sin bordes para pantalla completa
        is_resizable=False
    )
    return window
